const dadosPerguntas = {
    perguntas: [
      ...
      { textoPergunta: "Qual time é conhecido como Verdão?", opcoes: 
        ["Juventude", "Palmeiras", "Atlético Mineiro", "Grêmio"], 
        respostaCorreta: 1 },
    ...
    perguntaAtual.opcoes.forEach((opcao, indice) => {
      ...
      alternativaDiv.addEventListener("click", function () {
        if (indice === perguntaAtual.respostaCorreta) {
          ...
          pontuacao++;  // Incrementa a pontuação em caso de resposta correta
        } else {
  ...
  function carregarProximaPergunta() {
    ...
    if (indicePerguntaAtual < dadosPerguntas.perguntas.length) {
      ...
      exibirResultadoFinal();  // Exibe o resultado final ao término do quiz
    }
  }
  ...
  function exibirResultadoFinal() {
    ...
    mensagemResultado.innerText = `Você acertou ${pontuacao} de ${dadosPerguntas.perguntas.length} perguntas!`;
